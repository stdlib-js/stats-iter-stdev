...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterstdev, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if provided a non-numeric second argument', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			iterstdev( array2iterator( [ 1, 2, 3 ] ), value );
		};
	}
});

tape( 'the function computes the corrected sample standard deviation', function test( t ) {
	var arr;
	var v;
	...
	t.strictEqual( v, 0.8944271909999159, 'returns expected value' );
	t.end();
});

tape( 'the function computes the corrected sample standard deviation (known mean)', function test( t ) {
	var arr;
	var v;
	...
	// Tested against Julia:
	t.strictEqual( v, 0.816496580927726, 'returns expected value' );
	t.end();
});

tape( 'the function returns `null` if provided an "empty" iterator', function test( t ) {
	var v = iterstdev( iterEmpty() );
